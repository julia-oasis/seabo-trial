...
angular.module('seaboApp')
  .controller('CollectionCtrl', ['$scope', '$rootScope', '$routeParams', '$document', '$filter', '$http', 'ListsDataFactory', function ($scope, $rootScope, $routeParams, $document, $filter, $http, ListsDataFactory) {
      ...
      var getCollection = function() { 
        var collection = $filter('findById')($rootScope.collections, $routeParams.id);
              ...
              .then(function successCallback(response) {
                ...
                $http.jsonp('http://api.trove.nla.gov.au/result?' + query + '&s=' + start + '&n=100&zone=picture&encoding=json&key=8dirgt7b3oj0vqd4&callback=JSON_CALLBACK', {cache: true})
                        .then(function successCallback(response) {
                            ...
                            var sample = getRandomSubarray(pictures, 48);
                            var images = [];
